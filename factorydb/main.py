...
def main():
    ...
    employees_frame.to_csv('employees.csv', columns=columns, index=False, header=False)
    employees_with_elements = list(generate_employees_with_elements(employees))
    pd.DataFrame(data=employees_with_elements).to_csv('elements.csv', index=False, header=False)
